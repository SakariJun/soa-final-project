...
{
    $sql = "select account.UserID, user.Fullname, account.Username, user.Role, user.Email, account.Activated, account.RequestResetPassword from account, user where account.UserID = user.UserID and user.Role != N'Giám đốc' order by account.RequestResetPassword desc,  FIELD(user.Role,N'Giám đốc',N'Trưởng phòng',N'Nhân viên')";
...
function statisticsStatusByEmployee($UID)
{
    // Lương
    ...
    // Tổng Task đã giao trừ việc đã hủy của nhân viên
    $sql = 'SELECT * FROM task WHERE Status != "Canceled" AND (OfficerID = ? OR ManagerID = ?)';
    $stmt = $conn->prepare($sql);
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('s', $user['DepartmentID']);

    if (!$stmt->execute()) {
        return array('status' => false, 'errorMessage' => 'Lỗi thực thi');
    }
    $result = $stmt->get_result();
    $data['teamates'] = $result->num_rows - 1;

    // Số task hoàn thành bởi nhân viên này
    $sql = 'SELECT * FROM task WHERE Status = "Completed" AND OfficerID = ?';
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('s', $UID);
    ...
    $result = $stmt->get_result();
    $data['tasks-completed-total'] = $result->num_rows;

    // Lương thưởng
    // Số task hoàn thành bởi nhân viên này trong tháng
    $sql = 'SELECT * FROM task WHERE Status = "Completed" AND (OfficerID = ? OR ManagerID = ? ) AND MONTH(Deadline)=MONTH(CURRENT_DATE())';
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('ss', $UID, $UID);
    ...
    $result = $stmt->get_result();
    $taskCompleted = $result->num_rows;
    $data['tasks-completed-month'] = $result->num_rows;
    ...
    $data['bonus-salary'] = $user['Salary'] * ($taskCompleted + $bonus) / 100;
